    ...
    public class Program
    {
        static void Main(string[] args)
        {
            using (var context = new AppContext())
            {
                ...
                Console.WriteLine("\nПолучение жанра книги по ID:");
                if (Int32.TryParse(Console.ReadLine(), out int id) && id > 0 && id <= 5)
                {
                    var bookId = bookRepository.GetBookById(id);
                    Console.WriteLine($"Жанр книги {bookId.Title}: {bookId.Genre?.Name}");
                }
                
                Console.WriteLine("\nПодробные данные о книге 6:");
                var bookDetailedInfo = bookRepository.GetBookWithDetailsById(book6.Id);
                Console.WriteLine($"Книга: {bookDetailedInfo.Title}, Год: {bookDetailedInfo.Year}");
                
                Console.WriteLine("\nОбновленние года у книги номер 6");
                bookRepository.UpdateBookYear(book6.Id, 2014);
                
                Console.WriteLine("\nОбновленные подробные данные о книге 6:");
                var bookDetailedInfo2 = bookRepository.GetBookWithDetailsById(book6.Id);
                Console.WriteLine($"Книга: {bookDetailedInfo.Title}, Год: {bookDetailedInfo.Year}");
